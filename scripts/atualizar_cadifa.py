...
from diagnosticar_ifa_regularidade import (
    CADIFA_FONTES, descobrir_powerbi_cadifa, diagnosticar_painel,
...
def score_entidade(ent):
    ...
    p = 0
    for termo, peso in (("cadifa", 8), ("insumo", 4), ("ifa", 4), ("situac", 3), ("processo", 2), ("expediente", 2)):
        if termo in s:
            p += peso
    if "notifica" in s or "pos-registro" in s:
        p -= 8
    return p


def mapear_campos(campos):
    """Mapeia os nomes efetivamente publicados pelo painel CADIFA.

    Não cria número/revisão de CADIFA quando o modelo oficial não os publica.
    """
    preferidos = {
        "ifa": ["NO_INSUMO_MINUSC"],
        "detentor": ["NO_RAZAO_SOCIAL_MAISC"],
        "processo": ["NU_PROCESSO"],
        "expediente": ["NU_EXPEDIENTE"],
        "assunto": ["DS_ASSUNTO"],
        "codigo_assunto": ["CO_ASSUNTO"],
        "situacao": ["DS_SITUACAO_APRESENTACAO"],
        "apresentacao": ["DS_APRESENTACAO_PRODUTO"],
    }
    mapa = {}
    for destino, candidatos in preferidos.items():
        for c in candidatos:
            if c in campos:
                mapa[destino] = c
                break
            ...
            for bloco in ds.get("PH", []):
                for matriz in bloco.values():
                    ...
                    for linha in matriz:
                        ...
                        if linha.get("S"):
                            esquema = linha["S"]
                        cel = list(linha.get("C", []))
                        rep = int(linha.get("R", 0) or 0)
                        nul = int(linha.get("Ø", 0) or 0)
                        cur = 0
                        vals = []
                        for i in range(len(campos)):
                            ...
                            if i < len(esquema):
                                ...
                                if dn and isinstance(val, int):
                                    val = dics.get(dn, {}).get(val, val)
                            vals.append(val)
                        anterior = vals
                        if any(v not in (None, "") for v in vals):
                            saida.append(dict(zip(campos, vals)))
    return saida
...
def main():
    ...
    for url in urls:
        try:
            d = diagnosticar_painel("cadifa", url, CADIFA_FONTES[0])
            d["pontuacao_cadifa"] = pontuar_cadifa(d)
            candidatos.append(d)
        except Exception as e:
            print("Candidato ignorado:", url, type(e).__name__, e)
    if not candidatos:
        raise RuntimeError("Painel CADIFA não localizado a partir das páginas oficiais da Anvisa")
    candidatos.sort(key=lambda x: x.get("pontuacao_cadifa", 0), reverse=True)
    diag = candidatos[0]
    if diag.get("pontuacao_cadifa", 0) < 5:
        raise RuntimeError("Nenhum painel encontrado pôde ser identificado com segurança como CADIFA")
    ents = sorted(diag["entidades"], key=score_entidade, reverse=True)
    if not ents or score_entidade(ents[0]) < 8:
        raise RuntimeError("Entidade CADIFA não identificada com segurança no modelo")
    ent = ents[0]
    entidade = ent["entidade"]
    campos = ent["campos"]
    mapa = mapear_campos(campos)
    essenciais = ("ifa", "detentor", "processo", "situacao")
    faltantes = [x for x in essenciais if not mapa.get(x)]
    if faltantes:
        raise RuntimeError(f"Campos essenciais CADIFA ausentes: {faltantes}; mapa={mapa}; campos={campos}")
    ...
    h = headers_powerbi(diag["resource_key"], diag["pagina_powerbi"])
    ...
    registros_origem = decodificar(json.loads(rb.decode("utf-8", errors="replace")), campos)
    if not registros_origem:
        raise RuntimeError("Painel CADIFA retornou zero registros")

    registros = []
    for r in registros_origem:
        item = {k: r.get(k) for k in campos}
        for destino, origem in mapa.items():
            item[destino] = r.get(origem)
    ...
    banco = {
        "schema": "cadifa-v1",
        "gerado_em": gerado,
        "fonte_oficial": CADIFA_FONTES[0],
        "pagina_oficial_cadifa": CADIFA_FONTES[1],
        "painel_oficial": diag["pagina_powerbi"],
        "observacao": "Banco público para cruzamento regulatório. O painel consultado não publica, no modelo atual, campo explícito de número/revisão da CADIFA. A presença ou situação de um registro não comprova, isoladamente, a conformidade de fornecedor ou lote específico.",
        "entidade_origem": entidade,
        "campos_origem": campos,
        "mapa_campos": mapa,
        "campos_nao_disponiveis_no_modelo_atual": ["numero_cadifa_explicito", "revisao_cadifa_explicita"],
        "total": len(registros),
        "registros": registros,
    }
    ...
    manifest.setdefault("fontes", {})["cadifa"] = {
        "arquivo": "cadifa.json",
        "fonte_oficial": CADIFA_FONTES[0],
        "painel_oficial": diag["pagina_powerbi"],
        "total": len(registros),
        "sha256": banco["sha256_conteudo"],
        "mapa_campos": mapa,
        "status_querydata": status,
    }
    mf.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")

    (SAIDA / "cadifa-descoberta.json").write_text(json.dumps({
        "gerado_em": gerado,
        "painel_escolhido": diag["pagina_powerbi"],
        "pontuacao": diag["pontuacao_cadifa"],
        "entidade": entidade,
        "campos": campos,
        "mapa_campos": mapa,
        "descoberta": desc_meta,
