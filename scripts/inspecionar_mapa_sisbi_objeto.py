...
def main() -> int:
    ...
    try:
        with sync_playwright() as p:
            ...
            # Evita baixar recursos pesados que não são necessários para o diagnóstico.
            # Scripts e XHR/WebSocket continuam liberados.
            def bloquear_pesados(route):
            ...
            # O servidor do painel por vezes demora para encerrar o carregamento HTML.
            # "commit" basta para iniciar a execução do mashup; depois aguardamos
            # explicitamente a variável Qlik "app".
            for tentativa in range(1, 4):
                try:
                    ...
                    ultimo_erro = exc
                    console_msgs.append(
                        f"goto tentativa {tentativa}/3: "
                        f"{type(exc).__name__}: {exc}"
                    )
                    if tentativa < 3:
                        page.wait_for_timeout(tentativa * 5000)
            ...
            # Aguarda o próprio mashup público criar a aplicação Qlik.
